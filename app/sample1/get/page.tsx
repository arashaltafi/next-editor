...
const Sample1Get = () => {
    const [data, setData] = useState<Sample1 | null>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    const fetchData = async () => {
        try {
            const res = await fetch(Address.EndPoints.sample1Get);
            if (!res.ok) {
    ...
    return (
        <div className="w-full min-h-screen flex flex-col gap-8 items-center justify-start px-16 py-8">
            <h1 className='w-full text-center text-5xl font-bold'>Sample 1 Get</h1>
